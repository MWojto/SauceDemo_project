...
test("Login no username", async ({ page }) => {
    await page.goto('https://www.saucedemo.com/');
    await page.getByText('Swag Labs').isVisible();
    await page.locator('[data-test="login-button"]').isVisible();
    ...
    await page.locator('[data-test="password"]').fill('secret_sauce123');
    await page.locator('[data-test="login-button"]').click();
    await page.locator('div').filter({ hasText: /^Epic sadface: Username is required$/ }).isVisible();
    await page.close();
})
...
test("Login no password", async ({ page }) => {
    ...
    await page.locator('[data-test="login-button"]').click();
    await page.locator('div').filter({ hasText: /^Epic sadface: Password is required$/ }).isVisible();
    await page.close();
})

test("Login no data inserted", async ({ page }) => {
    await page.goto('https://www.saucedemo.com/');
    await page.getByText('Swag Labs').isVisible();
    await page.locator('[data-test="login-button"]').isVisible();
    await page.locator('[data-test="username"]').fill('');
